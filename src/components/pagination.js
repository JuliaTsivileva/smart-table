...
export const initPagination = ({pages, fromRow, toRow, totalRows}, createPage) => {
    ...
    return (data, state, action) => {
        ...
        const rowsPerPage = state.rowsPerPage;                        // шорт энтри будем часто обращаться, чтобы короче записывать
        const pageCount = Math.ceil(data.length / rowsPerPage);        // округление стр в большую сторону
